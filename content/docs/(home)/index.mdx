...
Pylon 基于 Rebar 构建——这是一个强大的框架，允许添加自定义方块、物品、研究等。Rebar 还实现了复杂的系统，如流体处理逻辑与货物路由。**Pylon 实际上是 Rebar 的一个附加组件**。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>🇺🇸 Seggan</td>
      <td>Slimefun 附加组件的资深开发者（SlimefunWarfare、SFCalc、Galactifun），为 Slimefun 附加、Paper 和 Slimefun 贡献了大量代码。Seggan 负责了 Rebar 与 Pylon 的许多核心系统，包括翻译、WAILA、研究、冶炼厂、配方系统等。</td>
    </tr>
    <tr>
      <td><span style={{color: '#ff0000'}}>首席开发者</span></td>
      <td>🇨🇦 Ohm</td>
    ...
    <tr>
      ...
      <td>🇨🇳 Balugaq</td>
      <td>前 Slimefun 附加开发者（JustEnoughGuide、SlimefunTimeIt、MSUA、AdvancedBan 等），为 Pylon/Rebar 贡献了代码（主要是配方原料计算系统），并已编写了两个扩展。</td>
    </tr>
    <tr>
      <td><span style={{color: '#bc915c'}}>贡献者</span></td>
      <td>🇺🇸 Blueb</td>
    ...
    <tr>
      <td><span style={{color: '#bc915c'}}>贡献者</span></td>
      <td>🇨🇳 Ybw0014</td>
      <td>前 Slimefun 附加开发者，曾管理 Slimefun 社区。帮助撰写了部分 Pylon/Rebar 文档。</td>
    </tr>
  </tbody>
</table>
...
- ⚙️ 轻松禁用任何有问题的方块或物品。
- ⚙️ 如果检测到配置问题，Rebar 或 Rebar 附加组件将拒绝启动。
- ⚙️ 抛出错误的方块会被安全卸载。
- ⚙️ 移除附加组件是安全的，所有数据将完好保留，如果重新添加该附加组件，数据将会恢复。
...
| **Rebar 会支持 Slimefun 附加组件吗？** | 不会。从 Slimefun 迁移到 Rebar 并非易事，我们建议附加开发者完全重写其附加内容，以更好地契合 Pylon 的进度与风格，而非尝试一对一迁移。                                                                                                                                                       |
